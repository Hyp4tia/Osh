        ...
        func exportHTML(webView: WKWebView, completion: @escaping (String?) -> Void) {
            webView.evaluateJavaScript("window.exportHTML()") { [weak self] result, error in
                ...
                if let error = error {
                    ...
                    let handler = webView.configuration.urlSchemeHandler(forURLScheme: "local-md") as? LocalSchemeHandler
                    let baseDir = handler?.baseDirectory ?? self.currentFileURL?.deletingLastPathComponent()
                    let allowedURLs = handler?.allowedFileURLs ?? []
                    
                    let inlinedHTML = HTMLExportInliner.inlineSafeLocalImages(
                        html: html,
                        baseDirectory: baseDir,
                        allowedFileURLs: allowedURLs
                    )
                    completion(inlinedHTML)
                } else {
                    completion(nil)
                }
            }
